...
- `POST /api/posts/{id}/view` — засчитать просмотр (дедупликация по сессии на 3 часа через Redis, см. [posts_architecture_ru.md](posts_architecture_ru.md)); возвращает `{ counted: true|false }`
